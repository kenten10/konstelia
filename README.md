...
- アンカー対象のコードを編集しただけではdriftedになりません。保存したsnapshotと同じコードが他の場所に残っている場合（序数がずれて別の宣言を指した場合）にdriftedとして検出します。
...
先頭では前へ進めません。最後のホップでは「次へ」の代わりに**完了**を表示し、選ぶとツアーを完了して通知します。完了せずに閉じたい場合は**終了**を選びます。コマンドパレットの**Konstelia: End Tour**でも終了できます。
...
未保存の変更があるとタブ名の先頭に`●`が付き、保存ボタンの横に状態を表示します。`Cmd+S` / `Ctrl+S`でも保存できます。ウィンドウを再読み込みしても、未保存の変更は復元されます。

ステップ・ホップ・アンカー・リンクの追加、削除、並べ替えは**元に戻す**（入力欄の外で`Cmd+Z` / `Ctrl+Z`）で取り消せます。入力欄の中では、その欄自身の取り消しが働きます。
...
![ツアー編集画面。左にステップとホップのフォーム、右に自動生成されたフロー図](media/konstelia-editor.png)
...
| **Konstelia: End Tour** | 再生中のツアーを終了する（再生中のみ表示） |
