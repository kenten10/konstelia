...
export class TourFlowDiagramView implements WebviewViewProvider, TourPlaybackObserver {
  ...
  public async show(tour: TourDocument, anchorHealth?: ReadonlyMap<string, AnchorHealth>): Promise<void> {
    this.source = {
      tour,
  ...
  private handle(message: ViewMessage): void {
    ...
    if (!position || !this.playbackSessionForShownTour()) {
      return;
    }
  ...
  private render(): void {
    ...
    const currentNodeId = state && state.status === "playing"
      ...
      : undefined;
    const layout = layoutTourFlowDiagram(this.source.diagram);
